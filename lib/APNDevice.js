...
APNDevice.prototype.APNDeviceRegisterOnDB = function APNDeviceRegisterOnDB(token,tags,startDateGMT,endDateGMT,deviceBadge,callback) {
  ...
  deviceDB.defaultDatabaseConnection(this.databaseInformations,function (connection) {
    ...
    deviceDB.createUpdate(connection,1,token,tags,startDateGMT,endDateGMT,deviceBadge,function (queryStatus,deviceID) {
      if (queryStatus) { 
        //Create tags in database
        deviceDB.createUpdateTags(connection,tags,function (queryStatus,respID) {
          if (queryStatus) {
            //Clean Tags associated with that device
            deviceDB.cleanTagsOfDevice(connection,deviceID,function (queryStatus,response) {
              if (queryStatus) {
                //Associate tags with device
                deviceDB.associateDeviceWithTag(connection,deviceID,respID,function (queryStatus,resp) {
                  if (queryStatus) { callback(true,'OK'); }
                  else { callback(false,'query error '+resp); }
                  deviceDB.logRegister(connection,deviceID,1,function (leStatus,response2) {
                    //Clean unused tags
                    deviceDB.cleanUnusedTags(connection,function(queryStatus,response3) { connection.end(); }) 
                  });
                });
...
APNDevice.prototype.APNDeviceUnregisterOnDB = function APNDeviceUnregisterOnDB(token,callback) {
  ...
  deviceDB.defaultDatabaseConnection(this.databaseInformations,function (connection) {
    //Create or Update device in database
    deviceDB.unregister(connection,token,function (queryStatus,deviceID) {
      if (queryStatus && deviceID) { 
        callback(true,'OK');
        deviceDB.logRegister(connection,deviceID,0,function (leStatus,response2) { connection.end(); });
      } else { callback(false,'query error '+deviceID+queryStatus); connection.end(); }
    });
  },function (err) { callback(false,err); });
}
...
APNDevice.prototype.APNDeviceListDeviceRegisterDataPointsOnDB = function APNDeviceListDeviceRegisterDataPointsOnDB(startDate,endDate,callback) {
  ...
  deviceDB.defaultDatabaseConnection(this.databaseInformations,function (connection) {
    ...
    deviceDB.listRegisterCalls(connection,startDate,endDate,function (queryStatus,resp) {
