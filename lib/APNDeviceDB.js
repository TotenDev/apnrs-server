...
* @param String callback.resp - response string|obj when convinient.
**/
APNDeviceDB.prototype.APNDeviceLogRegister = function APNDeviceLogRegister(connection,deviceID,registering,callback) {
...
APNDeviceDB.prototype.listRegisterCalls = function listRegisterCalls(connection,startDate,endDate,callback) {
  var strQuery = util.format('SELECT `deviceID`,`registering`,`createDate` FROM `%s` WHERE `createDate` >= `%s` AND `createDate` <= `%s` ',APNDeviceRegisterDatabaseMysqlKey,startDate,endDate);
  console.log(strQuery);
  var query = connection.query(strQuery, function(err, result) {
    console.log(strQuery,result,err);
    if ((!err || err == null) && result && result.length > 0) { callback(true,result); }
    else { callback(false,err); }
  });
} 
