...
APNMessage.prototype.APNMessageList = function APNMessageList(bodyData,callback) {
  ...
  if (order == 'DSC' || order == 'ASC') { 
     this.databaseWrapper.APNMessageListOnDB(order,callback);/*List messages in DB*/  
  }
...
APNMessage.prototype.APNMessagePrepare = function APNMessagePrepare(msg_alert,msg_sound,msg_badge,msg_broadcasting,msg_tags,msg_tokens,callback) {
  ...
  preparationQueue.push(function (nextStep) {
    ...
    if (msg_broadcasting && msg_broadcasting=='true') {
      instance.databaseWrapper.APNMessageListActiveDevicesOnDB(function (status,resp) {
        if (status == definitions.retValSuccess) { devices = resp; nextStep(); }
    ...
    /*if contains tags, get all tokens associated with that tokens*/
      ...
      instance.databaseWrapper.APNMessageListActiveDevicesWithTagsOnDB(msg_tags,function (status,resp) {
        if (status == definitions.retValSuccess) { devices = resp; nextStep(); }
        else { callback(status,resp); return; }
      });
    }
      ...
      if (!msg_tokens || msg_tokens.length == 0) { callback(definitions.retValAccept,'no tokens on body'); return; }
        ...
        for (token in msg_tokens) {
          instance.databaseWrapper.APNMessageTokenIsValid(msg_tokens[token],function (status,resp) {
            completed += 1;
            if (status == definitions.retValSuccess) { devices.push(resp); }
  ...
  preparationQueue.push(function (nextStep) {
    ...
    for (token in oldTokens) {
      ...
      //Set time
      startDate.setHours(oldTokens[token]['silentStartGMT'].substring(0,2));
      startDate.setMinutes(oldTokens[token]['silentStartGMT'].substring(3,5));
      startDate.setSeconds(oldTokens[token]['silentStartGMT'].substring(6,8));
      //
      endDate.setHours(oldTokens[token]['silentEndGMT'].substring(0,2));
      endDate.setMinutes(oldTokens[token]['silentEndGMT'].substring(3,5));
      endDate.setSeconds(oldTokens[token]['silentEndGMT'].substring(6,8));
      //if now is bigger than startDate
      if (now > startDate && now < endDate) { tokens.push(oldTokens[token]); }
      else { silentTokens.push(oldTokens[token]); }
    }
    nextStep();
  });
    ...
    messagesQueue.push(function (nextDevice,currentDevice) {
      ...
      if (msg['badge'] && msg['badge'].length > 0) {
       if (msg['badge'].match(/^[+-]{1}[0-9]{1,}/)) {
          var currentDeviceBadge = parseInt(currentDevice['badge']);
          var msgBadgeForOperation = parseInt(msg['badge'].replace(/^[-+]{1}/g, ""));
          if (msg['badge'].match(/^[+]{1}[0-9]{1,}/)) { currentDeviceBadge += msgBadgeForOperation; }
          else { currentDeviceBadge -= msgBadgeForOperation ; }
          //check limit
          if (currentDeviceBadge < 0) { currentDeviceBadge = 0; }
          msg['badge'] = currentDeviceBadge.toString();
        }else {
      ...
      //Update device badge
      instance.databaseWrapper.APNMessageUpdateDeviceBadge(currentDevice['token'],msg['badge'],function (status,resp) {});
      //Add into db (with status as `sending`)
      instance.databaseWrapper.APNMessageCreateLog(msg,info["broadcasting"],info["tags"],currentDevice['token'],function (status,resp) {
        if (status == definitions.retValSuccess) {
          var logID = resp;
          if (logID) {
            //send message to apple push servers
            server.sendMessage(currentDevice['token'],{aps:msg},function (notification) {
              if (notification) { 
