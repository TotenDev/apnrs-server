...
* @param String callback.resp - response string|obj when convinient.
**/
APNMessageQueryWrapper.prototype.APNMessageUpdateDeviceBadge = function APNMessageUpdateDeviceBadge(token,badge,callback) {
  //create database connection for listing
  queries.defaultDatabaseConnection(this.databaseInformations,function (connection) {
    //update device badge
    queries.updateDeviceBadge(connection,token,badge,function (status,resp) {
      definitions.respondQueryCallback(connection,callback,status,resp);
    });
  },callback);
}
...
* @param String order - order of this list (ASC|DSC) - REQUIRED
* @param Function callback - function callback - REQUIRED
* @param Boolean callback.okay - if filled okay (true) or with errored (false).
* @param String callback.resp - response string|obj when convinient.
**/
APNMessageQueryWrapper.prototype.APNMessageListOnDB = function APNMessageListOnDB(order,callback) {
  //create database connection for listing
  queries.defaultDatabaseConnection(this.databaseInformations,function (connection) {
    //list messages
    queries.listPushNotificationsMessage(connection,order,function (status,resp) {
      definitions.respondQueryCallback(connection,callback,status,resp);
    });
  },callback);
}
