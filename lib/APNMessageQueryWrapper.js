...
* @param String callback.resp - response string|obj when convinient.
**/
APNMessageQueryWrapper.prototype.APNMessageCreateLog = function APNMessageCreateLog(msg_alert,msg_sound,msg_badge,isBroadcasting,tags,deviceToken,status,sentDate,callback) {
  //create database connection for listing
  queries.defaultDatabaseConnection(this.databaseInformations,function (connection) {
    //crate push notifications log on db
    queries.createPushMessageLog(connection,msg_alert,msg_sound,msg_badge,isBroadcasting,tags,deviceToken,status,sentDate,function (queryStatus,resp) {
      if (queryStatus) {  callback(true,resp); }
      else { callback(false,'query error '+resp); }
      connection.end();
    });
...
APNMessageQueryWrapper.prototype.APNMessageUpdateLogStatus = function APNMessageUpdateLogStatus(logID,status,callback) {
  ...
  queries.defaultDatabaseConnection(this.databaseInformations,function (connection) {
    //crate push notifications log on db
    queries.updatePushMessageLogStatus(connection,logID,status,function (queryStatus,resp) {
      if (queryStatus) {  callback(true,resp); }
      else { callback(false,'query error '+resp); }
      connection.end();
    });
