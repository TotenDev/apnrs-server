...
function APNRestServer(options) {
    ...
    else if (!options["rest"] || !options["rest"]["clientSecretUser"] || options["rest"]["clientSecretUser"].length == 0) { assert.ok(false,"** APNRestServer ** options 'clientSecretUser' is **REQUIRED**, but is not specified."); }
    else if (!options["rest"]["serverSecretUser"] || options["rest"]["serverSecretUser"].length == 0) { assert.ok(false,"** APNRestServer ** options 'serverSecretUser' is **REQUIRED**, but is not specified."); }
    else if (!options["rest"]["commonSecretPass"] || options["rest"]["commonSecretPass"].length == 0) { assert.ok(false,"** APNRestServer ** options 'commonSecretPass' is **REQUIRED**, but is not specified."); }
    ...
    APNRestServerInstance.restPrivateKey = fs.readFileSync('./../dev/privatekey.pem').toString();
    APNRestServerInstance.restCertificate = fs.readFileSync('./../dev/certificate.pem').toString();
    //Start REST Server
