...
APNRestServer.prototype.receiveRequest = function receiveRequest(req, res, next) {
 //Receive data
  ...
  req.on('end',function () {
    if (!errored && !APNRestServerInstance.routeRequest(req,data,APNRestServerInstance.getClientIp(req),res)) { res.send(404,new Error("route not found :(")); }
  });
}
//Main Route
APNRestServer.prototype.routeRequest = function routeRequest(request, bodyData, clientIP,response) {
  ...
  //        res.send(200);
          ...
          currentDevice.APNDeviceRegister(token,tags,startDate,endDate,timeZone,function (okay,functionResponse) {
            if (okay) { response.send(200,new Error(functionResponse)); }
            else { response.send(500,new Error(functionResponse)); }
          });
        }
      }break;
      ...
      default: { 
        if(isRequestAuthenticated(request,response,true)){ response.send(404,new Error("Not route found.")); }
      } break;
    }
    return true;
}