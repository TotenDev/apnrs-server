...
function APNRestServer(options) {
    ...
    else if (!options["rest"] || !options["rest"]["clientSecretUser"] || options["rest"]["clientSecretUser"].length == 0) { assert.ok(false,"** APNRestServer ** options 'rest.clientSecretUser' is **REQUIRED**, but is not specified."); }
    else if (!options["rest"]["serverSecretUser"] || options["rest"]["serverSecretUser"].length == 0) { assert.ok(false,"** APNRestServer ** options 'rest.serverSecretUser' is **REQUIRED**, but is not specified."); }
    else if (!options["rest"]["commonSecretPass"] || options["rest"]["commonSecretPass"].length == 0) { assert.ok(false,"** APNRestServer ** options 'rest.commonSecretPass' is **REQUIRED**, but is not specified."); }
    else if (!options["rest"]["cert"] || options["rest"]["cert"].length == 0) { assert.ok(false,"** APNRestServer ** options 'rest.cert' is **REQUIRED**, but is not specified."); }
    else if (!options["rest"]["key"] || options["rest"]["key"].length == 0) { assert.ok(false,"** APNRestServer ** options 'rest.key' is **REQUIRED**, but is not specified."); }
    ...
    //Get required values
    APNRestServerInstance.clientSecretUser = options["rest"]["clientSecretUser"];
    APNRestServerInstance.serverSecretUser = options["rest"]["serverSecretUser"];
    APNRestServerInstance.commonSecretPass = options["rest"]["commonSecretPass"];
    APNRestServerInstance.database = options["database"];
    APNRestServerInstance.push = options["push"];
    //Optional options
    APNRestServerInstance.serverPort = (process.env.PORT || options["rest"]["serverPort"] || 8080);
    APNRestServerInstance.restRequestLimit = (options["rest"]["requestLimit"] || -1);
    APNRestServerInstance.restPrivateKey = options["rest"]["key"];
    APNRestServerInstance.restCertificate = options["rest"]["cert"];
    //Start REST Server
...
APNRestServer.prototype.startRestServer = function startRestServer() {
  APNRestServerInstance.server = server = restify.createServer({
    name:"APNRS",
    certificate:APNRestServerInstance.restCertificate,
    key:APNRestServerInstance.restPrivateKey
  });
  //throttle module setup (optionally)
  if (APNRestServerInstance.restRequestLimit != -1) {
   server.use(restify.throttle({
      burst: APNRestServerInstance.restRequestLimit,
      rate: APNRestServerInstance.restRequestLimit,
      ip: true
    })); 
  }
  //Server checks
  ...
  server.listen(APNRestServerInstance.serverPort , function() {
    console.log('**REST Server** %s listening at %s' ,APNRestServerInstance.server.name, APNRestServerInstance.server.url);
  });
}
