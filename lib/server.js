...
APNRestServer.prototype.authenticateRequest = function authenticateRequest(req,res,next) {
  ...
  if(!req.headers["authorization"]) { res.send(401,new Error('{"error":"unauthorized :("')); }
    ...
    if (auth.length >= 2) {
      ...
      return next();
    }else { res.send(401,new definition.encodeErrorToJSON("unauthorized :(")); return; }
  }
}
...
APNRestServer.prototype.receiveRequest = function receiveRequest(req, res, next) {
 //Receive data
  ...
  req.on('end',function () {
    if (!errored && !APNRestServerInstance.routeRequest(req,data,APNRestServerInstance.getClientIp(req),res)) { res.send(404,definition.encodeErrorToJSON("route not found :(")); }
  });
}
//Main Route
APNRestServer.prototype.routeRequest = function routeRequest(request, bodyData, clientIP,response) {
    ...
    function isRequestAuthenticated(req,res,server) {
      if (req.username != (server ? APNRestServerInstance.serverSecretUser : APNRestServerInstance.clientSecretUser) || req.password != APNRestServerInstance.commonSecretPass) {
        if (server) res.send(403,definition.encodeErrorToJSON('forbidden :('));
        return false;
      }else { return true; }
    }
    ...
    switch (request.path) {
      ...
      case "/register/": case "/register": {
        if(isRequestAuthenticated(request,response,false) || isRequestAuthenticated(request,response,true)){
            ...
            currentDevice.APNDeviceRegister(bodyData,function (status,functionResponse) { respondRequest(response,status,functionResponse); });
        }
      }break;
      //Send push message
      //Method: POST
...
//          message().APNMessagePrepareAndSend()
//          APNMessagePrepareAndSend(msg_alert,msg_sound,msg_badge,msg_broadcasting,msg_tags,msg_tokens,callback) {
            //DEBUG
            response.send(200);
        }
      }break;
      ...
      case "/stats/devices": case "/stats/devices/": {
        if(isRequestAuthenticated(request,response,true)){
          var currentDevice = device(APNRestServerInstance.database);
          currentDevice.APNDeviceListDeviceRegisterDataPoints(bodyData,function (status,functionResponse) { respondRequest(response,status,functionResponse); });
        }
      }break;
      ...
      case "/list/devices/": case "/list/devices": {
        if(isRequestAuthenticated(request,response,true)){
          var currentDevice = device(APNRestServerInstance.database);
          currentDevice.APNDeviceList(bodyData,function (status,functionResponse) { respondRequest(response,status,functionResponse); });
        }
      }break;
      ...
      case "/list/tags/": case "/list/tags": {
        if(isRequestAuthenticated(request,response,true)){
          var currentDevice = device(APNRestServerInstance.database);
          currentDevice.APNDeviceListTags(order,function (status,functionResponse) { respondRequest(response,status,functionResponse); });
        }
      }break;
      ...
      default: { 
        if(isRequestAuthenticated(request,response,true)){ response.send(404,definition.encodeErrorToJSON("route not found :(")); }
      } break;
    }
    return true;
}