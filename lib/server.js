        ...
        if(isRequestAuthenticated(request,response,false) || isRequestAuthenticated(request,response,true)){
          ...
          if (bodyData && bodyData['token'] && bodyData['token'].length > 0) {
            var token = bodyData['token'].replace("-","").toLowerCase(),
                ...
                tags = null;
            //optional values
            if (bodyData['startDate'] && bodyData['startDate'].length == 5) { startDate = bodyData['startDate']; }
            else { startDate = '09:00'; /*Defaults*/ }
            if (bodyData['endDate'] && bodyData['endDate'].length == 5) { endDate = bodyData['endDate']; }
            else { endDate = '23:00'; /*Defaults*/ }
            if (bodyData['timezone'] && bodyData['timezone'].length == 5) { timeZone = bodyData['timezone']; }
