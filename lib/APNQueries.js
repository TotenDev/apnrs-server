...
* @param String callback.resp - response string|obj when convinient.
**/
APNQueries.prototype.listPushNotificationsMessage = function listPushNotificationsMessage(connection,callback) {
  var strQuery = util.format('SELECT `msg_alert`,`msg_sound`,`msg_badge`,`isBroadcasting`,`tags`,`deviceToken`,`status` FROM `%s` WHERE 1',APNPushMessagesDatabaseMysqlKey);
  var query = connection.query(strQuery, function(err, result) {
...
APNQueries.prototype.defaultDatabaseConnection = function defaultDatabaseConnection(databaseInfo,successCb,errorCb) {
  ...
  connection.connect(function(err) {
    if (err) { console.log(err); errorCb(definitions.retValError,'Error in connecting to mysql database: '+err); }
