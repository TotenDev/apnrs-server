...
APNQueries.prototype.listFeedback = function listFeedback(connection,order,callback) {
  ...
  var query = connection.query(strQuery, function(err, result) {
    ...
    else { callback(definitions.retValError,err); }
  });
}
...
APNQueries.prototype.createPushMessageLog = function createPushMessageLog(connection,msg,isBroadcasting,tags,deviceToken,status,callback) {
  var strQuery = util.format('INSERT INTO %s (msg_alert,msg_sound,msg_badge,isBroadcasting,tags,deviceToken,status) VALUES (\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\') ',APNPushMessagesDatabaseMysqlKey,msg["alert"],msg["sound"],msg["sound"],isBroadcasting,tags,deviceToken,status);
...
APNQueries.prototype.listPushNotificationsMessage = function listPushNotificationsMessage(connection,order,callback) {
  var strQuery = util.format('SELECT `msg_alert`,`msg_sound`,`msg_badge`,`isBroadcasting`,`tags`,`deviceToken`,`status` FROM `%s` WHERE 1  ORDER BY `sentDate` %s',APNPushMessagesDatabaseMysqlKey,order);
  var query = connection.query(strQuery, function(err, result) {
...
APNQueries.prototype.listPushNotificationDatapoints = function listPushNotificationDatapoints(connection,startDate,endDate,callback) {
  var strQuery = util.format('SELECT `msg_alert`,`msg_sound`,`msg_badge`,`isBroadcasting`,`tags`,`deviceToken`,`status` FROM `%s` WHERE `sentDate` BETWEEN \'%s\' AND \'%s\' ',APNPushMessagesDatabaseMysqlKey,startDate,endDate);
  var query = connection.query(strQuery, function(err, result) {
    if ((!err || err == null) && result && result.length > 0) { callback(definitions.retValSuccess,result); }
    else { callback(definitions.retValError,err); }
  });
...
APNQueries.prototype.createUpdateDevices = function createUpdateDevices(connection,active,token,tags,startDateGMT,endDateGMT,deviceBadge,callback) {
  var strQuery = util.format('INSERT INTO %s (active,token,silentStartGMT,silentEndGMT,deviceBadge) VALUES (%s,\'%s\',\'%s\',\'%s\',%s) ON DUPLICATE KEY UPDATE active=%s,token=\'%s\',silentStartGMT=\'%s\',silentEndGMT=\'%s\',deviceBadge=%s,lastRegisterDate=now() ',APNDeviceDatabaseMysqlKey,active,token,startDateGMT,endDateGMT,deviceBadge,active,token,startDateGMT,endDateGMT,deviceBadge);
  var instance = this;
  //Insert Device
  var query = connection.query(strQuery, function(err, result) {
    if ((!err || err == null) && result.insertId != undefined) { 
...
APNQueries.prototype.updateDeviceBadge = function updateDeviceBadge(connection,token,badge,callback) {
  var strQuery = util.format('UPDATE %s SET deviceBadge=%s WHERE token=\'%s\'',APNDeviceDatabaseMysqlKey,badge,token);
  var instance = this;
  //Insert Device
  var query = connection.query(strQuery, function(err, result) {
    if ((!err || err == null) && result.insertId != undefined) { 
    ...
    if ((!err || err == null) && tagsID) {
      if (tagsID.length <= 0) {
        instance.associatedDevicesWithTagsID(connection,tagsID,function (err2,devicesID) {
          if ((!err2 || err2 == null) && (err2 == definitions.retValSuccess)) {
            if (devicesID.length <= 0) {
              ...
              var query = connection.query(strQuery, function(err3, result) {
                if ((!err3 || err3 == null) && result[0]) { callback(definitions.retValSuccess,result[0]); }
                else { callback(definitions.retValError,result); }
              });
            }else { callback(definitions.retValError,'no devices found with respective tags'); }
          }else { callback(err2,devicesID); }
        });
      }else { callback(definitions.retValError,'no tags found with respective names'); }
    }else { callback(err,tagsID); }
  });
}  
/**
      ...
      var strQuery = util.format('INSERT INTO %s (tag) VALUES (\'%s\') ON DUPLICATE KEY UPDATE tag=\'%s\'',APNTagsDatabaseMysqlKey,tags[tag],tags[tag]);
        var query = connection.query(strQuery, function(err, result) { 
          ...
          if ((!err || err == null) && result.insertId != undefined) { 
            ...
            if (lenght == current) { callback(definitions.retValError,err); }
            nextTag();
          }
        });
    });
...
APNQueries.prototype.associateDeviceWithTag = function associateDeviceWithTag(connection,deviceID,tagIDs,callback) {
  ...
  for (tagID in tagIDs) {
    ...
    var query = connection.query(strQuery, function(err, result) { 
      ...
      } else if (lenght == current) { callback(definitions.retValError,err); }
    });
  }
} 
/**
