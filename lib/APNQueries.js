...
* @param String status - message status (usually in creation of a message push it should be as starting or sending or some like that) - REQUIRED
...
**/
APNQueries.prototype.createPushMessageLog = function createPushMessageLog(connection,msg_alert,msg_sound,msg_badge,isBroadcasting,tags,deviceToken,status,sentDate,callback) {
  var strQuery = util.format('INSERT INTO %s (msg_alert,msg_sound,msg_badge,isBroadcasting,tags,deviceToken,status) VALUES (\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\') ',APNPushMessagesDatabaseMysqlKey,msg_alert,msg_sound,msg_badge,isBroadcasting,tags,deviceToken,status);
  ...
  var query = connection.query(strQuery, function(err, result) {
    if ((!err || err == null) && result.insertId != undefined) { 
      ...
      if (result.insertId == 0) { callback(false,result); }
...
APNQueries.prototype.updatePushMessageLogStatus = function updatePushMessageLogStatus(connection,logID,status,callback) {
  ...
  var query = connection.query(strQuery, function(err, result) {
    if ((!err || err == null) && result.insertId != undefined) { 
      //Check if not inserted and not updated (because no changes were necessary and it really exists on DB)
      if (result.insertId == 0) { callback(false,result); }
      else if (result.insertId == logID) { callback(true,result.insertId); }
      else { callback(false,'FATAL ERROR in updating push message status on DB'); }
    } 
    else { callback(false,result); }
  });
} 
